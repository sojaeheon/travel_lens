...
const fetchMyFavorites = async () => {
  try {
    ...
    const response = await axios.get('interaction/favorites/');
    const data = response.data?.results ?? response.data ?? []; // [{iso2, name_en, name_ko}, ...]
...
const handleRemoveFavorite = async (iso2) => {
  ...
  try {
    ...
    await axios.post('interaction/logs/', {
